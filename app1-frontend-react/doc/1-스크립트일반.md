# 자바스크립트 일반
React 개발에 있어서는 자바스크립트(ES6+ 중심) 문법에 대한 이해가 필수다.
 
## 1. `const`, `let`, `var` 변수 선언
### `var`
- 특징
  - 함수 스코프: 함수 안에서 선언되면 그 함수 안에서 유효하다.
  - 변수 호이스팅: 변수 선언이 코드 상단으로 끌어올려진다.
  - 중복 선언 가능
  - 재할당 가능
- 예시
    ```javascript
    function example() {
      console.log(a);    // undefined
      var a = 10;
      console.log(a);    // 10
    }
    ```
### `let`
- 특징
  - 블록 스코프: `{}`블록 내에서만 유효
  - 중복 선언 불가
  - 재핟당 가능
- 예시
    ```javascript
    function example() {
      let a = 10;
      a = 20;            // 재할당 가능
      if (a == 20) {
        let b = 30;
        console.log(b);   // 10
      }
      // console.log(b);  // ReferenceError: b is not defined
    }
    ```

### `const`
- 특징
  - 블록 스코프: `let`과 동일
  - 중복선언 불가
  - 재할당 불가
  - 선언시 반드시 초기화 필요
  - 객체/배열의 내부값은 수정 가능
- 예시
    ```javascript
    const x = 10;
    // x = 20;            // TypeError: Assignment to constant variable

    const user = { name: "홍길동" };
    user.name = "강감찬";  // 객체 속성은 변경 가 
    ```
 
## 2. 화살표 함수 (`=>`)
- 설명
  - 화살표 함수(Arrow Function)은 ES6(ECMAScript 2015)에서 도입된 함수 표현 방식이다.
  - 기존의 `function` 키워드를 사용하는 방식보다 간결한 문법을 제공한다.
- 기본 문법
    ```javascript
    const 함수이름 = (매개변수) => {
      // 함수 본문
    }
    ```
- 작성 예시
    ```javascript
    // 매개변수 1개일 경우 괄호 생략 가능
    const example = x => x + 1;
    ```
    
    ```javascript
    // 매개변수가 없을 경우 괄호 반드시 사용
    const example = () => console.log('Hello!');
    ```
    
    ```javascript
    // 함수 본문이 한 줄 표현식일 때만 중괄호와 return 생략 가능
    const example = (a, b) => a + b;    // return 생략가능

    // {}를 사용하면 블록으로 해석되기 때문에 return을 생략할 수 없다.
    const example = (a, b) => { return a + b;}
    ```
    
    ```javascript
    // 화살표 함수는 고유한 this를 가지지 않는다.
    // 화살표 함수에서 this를 참조하면, 화살표 함수가 아닌 평범한 외부 함수에서 this값을 가져온다.
    const obj = {
      name: "홍길동",
      getName: function() {
        let fn1 = function() {
          console.log(this.name);  // '', this는 window다.
        }
        let fn2 = () => {
          console.log(this.name);  // '홍길동', this는 외부함수 getName의 this다.
        }
        fn1();
        fn2();
      }
    }
    ```
    
    ```javascript
    // 화살표 함수는 생성자 함수로 사용할 수 없다.
    const Person = (name, email, tel) => {
      this.name = name;
      this.email = email;
      this.tel = tel;
    }

    // 화살표 함수는 new 키워드와 같이 사용할 수 없다.
    // const person1 = new Person("김유신", "kim@gmail.com", "010-1234-5678");  // TypeError
    ```
    
    ```javascript
    // 화살표 함수는 arguments 객체가 없음
    const example = () => {
      console.log(arguments);  // ReferenceError, 오류 발생
    }
    example(1, 2, 3);

    // 나머지 매개변수는 인수의 갯수의 제약이 없다.
    const example = (...numbers) => {
      console.log(numbers);    // [1, 2, 3]
    }
    example(1, 2, 3);
    ```
## 3. 템플릿 리터럴 (` `` `)
- 설명
  - 문자열과 변수, 표현식을 결합한다.
  - 여러 줄의 문자열을 작성할 수 있다.
- 예시
    ```jsx
    const name = "홍길동";

    const greeting = `${name}님 반갑습니다.`;
    return (
      <h1>{`Hello, ${name}`}</h1>
    )
    ```

## 4. 객체 리터럴 단축 문법
- 설명
  - 변수와 키 이름이 동일할 때 코드를 더 간결하고 가독성 좋게 만든다.
- 예시
    ```javascript
    // 기존 방식
    const name = "홍길동";
    const age = 30;

    const person = {
      name: name,
      age: age,
      sayHello: function() {
        console.log("Hello");
      }
    }

    // 단축 문법 - 키와 변수명이 같으면 name:name -> name으로 축약가능
    //            function 키워드 생략 가 
    const name = "홍길동";
    const age = 30;

    const person = {
      name,
      age,
      sayHello() {
        console.log("Hello");
      }
    }
    ```
## 5. 비구조화 할당
- 설명
  - 배열이나 객체의 값을 간단히 변수로 추출할 수 있는 문법이다.
  - 코드의 가독성돠 간결함을 유지할 수 있다.
- 예시
  - 배열의 비구조화 할당
    ```javascript
    // 기본 사용법
    const names = ["홍길동", "김유신", "강감찬"];
    const [a, b, c] = names;

    console.log(a);  // "홍길동"
    console.log(b);  // "김유신"
    console.log(c);  // "강감찬"
    ```

    ```javascript
    // 일부만 추출하기
    const names = ["홍길동", "김유신", "강감찬"];
    const [a, , b] = names;

    console.log(a);  // "홍길동"
    console.log(b);  // "강감찬"
    ```

    ```javascript
    // 기본값 지정하기
    const numbers = [10, 15];
    const [a = 0, b = 0, c = 0] = numbers;
    
    console.log(a);  // 10
    console.log(b);  // 15
    console.log(c);  // 0
    ```
  - 객체 비구조화 할당
    ```javascript
    // 기본 사용법
    const  user = { name: "홍길동", age: 30 };
    const { name, age } = user;

    console.log(name);  // "홍길동"
    console.log(age);   // 30 
    ```

    ```javascript
    // 다른 변수명으로 값 받기
    const  user = { name: "홍길동", age: 30 };
    const { name: userName, age: userAge } = user;

    console.log(userName);  // "홍길동"
    console.log(userAge);   // 30 
    ```
    
    ```javascript
    // 기본값 설정
    const  user = { name: "홍길동", age: 30 };
    const { name, age, deleted = false } = user;

    console.log(name);    // "홍길동"
    console.log(age);     // 30
    console.log(deleted); // false
    ```

    ```javascript
    // 중첩 객체 구조 분해
    const user = {
      name: "홍길동",
      address: {
        zip: "12345",
        city: "서울"
      }
    };

    const {name, address: {city, zip}} = user;

    console.log(name);  // "홍길동"
    console.log(city);  // "서울"
    console.log(zip);   // "12345"
    ```
  - 함수 매개변수에서 비구조화 할당
    ```javascript
    // 객체 매개변수
    function print({ name, age }) {
      console.log(name);  // "홍길동"
      console.log(age);   // 30
    }

    const user = { name:"홍길동", age:30 };
    print(user);
    ```

    ```javascript
    // 배열 매개변수
    function print([a, b, c]) {
      console.log(a);  // 10
      console.log(b);  // 20
      console.log(c);  // 30
    }

    print([10, 20, 30])
    ```
## 6. 전개 연산자 (`...`)
- 설명
  - 전개 연산자는 배열이나 객체, 문자열 등의 요소를 풀어서 복사하거나 결합할 때 사용하는 연산자다.
- 예시
  - 배열에서의 전개 연산자
    ```javascript
    // 배열 복사
    const nums = [1, 2, 3];
    const copy = [...nums];

    console.log(copy);  // [1, 2, 3]
    ```

    ```javascript
    // 배열 합치기
    const arr1 = [1, 2, 3];
    const arr2 = [4, 5];
    const merged = [...arr1, ...arr2]

    console.log(merged); // [1, 2, 3, 4, 5]
    ```

    ```javascript
    // 배열 요소 추가/삽입
    const arr1 = [2, 3];
    const extended = [1, ...arr1, 5];

    console.log(extended); // [1, 2, 3, 5]
    ```
  - 객체에서의 전개 연산자
    ```javascript
    // 객체 복사
    const user = { name: "홍길동", age: 30 };
    const copyUser = {...user};

    console.log(copyUser); // { name: "홍길동", age: 30}
    ```

    ```javascript
    // 객체 병합
    const obj1 = { name: "홍길동", age: 30 };
    const obj2 = { dept: "기술팀", grade:"초급" };
    const merged = { ...obj1, ...obj2 };

    console.log(merged); // { name: "홍길동", age: 30, dept: "기술팀", grade:"초급" }
    ```

    ```javascript
    // 새로운 속성 추가
    const obj1 = { name: "홍길동", age: 30 };
    const updated = { ...obj1, dept:"기술팀" };

    console.log(updated); // { name: "홍길동", age: 30, dept:"기술팀" }
    ```

## 7. 단락 평가 논리 연산자 (`&&`)
- 설명
  - 단락 평가 논리 연산자는 boolean 값뿐 아니라 일반 값에서도 작동한다.
  - 평가 도중 결과가 확정되면 남은 피연산자를 평가하지 않는다.
  - `||` 연산자 - 좌항이 falsy면 우항 반환
    - 평가 방식
      - 좌항이 true면 -> 좌항을 반환
      - 좌항이 false면 -> 우항를 반환
    - 기본값을 지정하는 용도로 사용한다. 
  - `&&` 연산자 - 좌항이 falsy가 아니면 우항 반환
    - 평가 방식
      - 좌항이 falsy면 -> 좌항을 반환
      - 좌항이 true면 -> 우항를 반환
    - 조건이 맞을 때만 실행할 때 사용한다.
- 예시
    ```javascript
    // ||로 기본값 설정하는 용도로 사용하기
    function greet(name) {
      const username = name || "손님";

      console.log(`${username}, 환영합니다.`);
    }

    greet();         // "손님, 환영합니다."
    greet("홍길동");  // "홍길동, 환영합니다." 
    ```

    ```javascript
    // &&로 조건이 맞을 때만 실행시키기
    const isLoggedIn = true;
    isLoggedIn && console.log("로그인되었습니다."); // "로그인되었습니다"
    ```

## 8. 모듈 시스템 (import/export)
- 설명
  - 자바스크립트의 모듈 시스템으 코드를 여러 파일로 분리하고, 필요한 코드만 가져와 재사용할 수 있도록 하는 구조다.
  - 모듈 시스템을 사용하면 코드의 재사용성, 유지보수성, 의존성 관리, 네임스페이스 충돌 방지 등의 이점이 있다.
  - `export` 키워드: 모듈에서 외부로 내보내는 코드다.
  - `import` 키워드: 다른 모듈에서 가져오는 코드다.
- 예시
    - Named Export (이름이 있는 내보내기)
      - 하나의 파일에서 여러 개의 값을 이름을 붙여서 export할 수 있다.
      - import할 때도 이름을 정확히 지정해서 가져와야 함
      - 중괄호 `{}`를 사용해서 가져온다.
      ```javascript
      // utils.js
      export const add = (a, b) => a + b;
      export const PI = 3.14;
      ```  
      ```javascript
      // main.js
      import { add,  PI } from './utils.js';
  
      console.log(add(10, 20));  // 30
      console.log(PI);           // 3.14
      ```
    - Default Export(기본 내보내기)
      - 한 파일에서 하나의 기본값만 export 할 수 있다.
      - import할 때 이름을 자유롭게 지정할 수 있다.
      - import 시 중괄호 {} 없이 사용한다.
      ```javascript
      // greet.js
      export default funtion greeting(name) {
        return `Hello, ${name}`;
      }
      ```
      ```javascript
      // main.js
      import greet from `greet.js`;

      console.log(greet("홍길동"));  // "Hello, 홍길동"
      ```
     
