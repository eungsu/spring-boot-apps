...
| **업데이트** | `useState`훅의 업데이트함수를 통해 명시적으로 변경한다.<br>State가 변경되면 해당 컴포넌트와 그 자식 컴포넌트가 다시 렌더링된다. | 부모 컴포넌트에서 전달하는 Props의 값이 변경되면, 해당 Props를 전달받은 자식 컴포넌트가 자동으로 다시 렌더링된다.|
| **데이터흐름** |||
