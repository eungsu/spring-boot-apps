...
│   ├── components/
│   │   ├── comments/
│   │   │   ├── CommentList.jsx                # 댓글 목록을 표시하는 컴포넌트다.
│   │   │   └── CommentForm.jsx                # 댓글 등록폼을 표시하는 컴포넌트다.
│   │   ├── posts/
...
│   │   └── CommentLayout.jsx                  # 댓글목록, 댓글등록폼을 포함하는 컴포넌트다.
│   ├── pages/
│   │   ├── posts/
...
| `ContentLayout` | 각 페이지의 내용을 담는 공통 레이아웃을 정의한다. 실제 페이지가 이 영역 안에서 렌더링된다. |
| `CommentLayout` | 댓글목록과 댓글등록폼을 담는 공통 레이아웃을 정의한다. |
...
> 예: `compontents/Login`, `components/Signup`, `components/Pagination`, `components/posts/PostList`, `components/posts/PostDetail`, `components/posts/PostForm`, `components/posts/PostUpdateForm`, `components/comments/CommentList`, `components/comments/CommentForm` 등    
 ...
 └── BaseLayout
      ...
      └── ContentLayout
           └── DetailPage              # 게시글 상세 페이지를 표시한다.
                ...
                └── CommentLayout      # 댓글 목록과 댓글입력폼을 포함하는 컴포넌트다.
                     ├── CommentList   # 댓글 목록을 표시하는 컴포넌트다.
                     └── CommentForm   # 댓글입력폼을 포시하는 컴포넌트다.
```
